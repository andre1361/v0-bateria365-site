import type { Metadata } from "next"
import { and, eq } from "drizzle-orm"
import { CalendarDays, MapPin, BookOpen, Building2 } from "lucide-react"
import { db } from "@/db"
import { events, companies, rsvps, users } from "@/db/schema"
import { InvitePreview } from "@/app/convites/invite-preview"
import { TeamClient } from "./team-client"

export const metadata: Metadata = {
  title: "Cadastro de equipe",
  robots: { index: false, follow: false },
}

function fmtDate(iso: string) {
  const p = (iso || "").split("-")
  return p.length === 3 ? `${p[2]}/${p[1]}/${p[0]}` : iso || ""
}

function norm(s: string) {
  return (s || "")
    .trim()
    .toLowerCase()
    .normalize("NFD")
    .replace(new RegExp("[\\u0300-\\u036f]", "g"), "")
}

function NotFound({ msg }: { msg: string }) {
  return (
    <div style={{ minHeight: "100vh", background: "#eef1f6", display: "flex", alignItems: "center", justifyContent: "center", padding: 24 }}>
      <div style={{ background: "#fff", border: "1px solid #e3e7ee", borderRadius: 18, padding: 34, textAlign: "center", maxWidth: 420 }}>
        <h2 style={{ margin: "0 0 6px", fontSize: 19, fontWeight: 800 }}>Link indisponível</h2>
        <p style={{ margin: 0, fontSize: 13.5, color: "#6a7585" }}>{msg}</p>
      </div>
    </div>
  )
}

export default async function EquipePage({ params }: { params: Promise<{ slug: string; companyId: string }> }) {
  const { slug, companyId } = await params

  const [row] = await db
    .select({ ev: events, distNome: users.nome })
    .from(events)
    .innerJoin(users, eq(users.id, events.distributorId))
    .where(and(eq(events.slug, slug), eq(events.ativo, true)))
  if (!row) return <NotFound msg="Este convite não existe ou foi desativado." />

  const ev = row.ev
  const [co] = await db
    .select()
    .from(companies)
    .where(and(eq(companies.id, companyId), eq(companies.distributorId, ev.distributorId)))
  if (!co) return <NotFound msg="Empresa não encontrada para este convite." />

  // Quantos já foram cadastrados por esta empresa neste evento.
  const allr = await db.select({ nome: rsvps.nome, empresa: rsvps.empresa }).from(rsvps).where(eq(rsvps.eventId, ev.id))
  const jaCount = allr.filter((r) => norm(r.empresa) === norm(co.nome)).length

  const dataFmt = fmtDate(ev.dataISO)
  const artData =
    ev.template === "vertical"
      ? { cidade: ev.cidade, data: dataFmt }
      : {
          cidade: ev.cidade,
          data: dataFmt,
          horario: ev.horario,
          distribuidor: row.distNome,
          local: ev.local,
          offBadge: -22,
          offTitle: 10,
          offPill: 24,
          offGroup: 86,
          offLogoX: 0,
          offLogoY: 12,
        }

  const detalhes = [
    ev.dataISO ? { icon: CalendarDays, label: "Data", value: dataFmt + (ev.horario ? ` · ${ev.horario}` : "") } : null,
    ev.local ? { icon: MapPin, label: "Local", value: ev.local } : null,
    ev.modulo ? { icon: BookOpen, label: "Módulo", value: ev.modulo } : null,
  ].filter(Boolean) as { icon: typeof CalendarDays; label: string; value: string }[]

  return (
    <div style={{ minHeight: "100vh", background: "linear-gradient(180deg,#eef1f6 0%,#dde6f2 100%)", color: "#1f2733" }}>
      <div style={{ height: 64, background: "#04377f", display: "flex", alignItems: "center", justifyContent: "center", padding: "0 16px" }}>
        {/* eslint-disable-next-line @next/next/no-img-element */}
        <img src="/images/logo-bateria365-escuro.png" alt="Bateria 365" style={{ height: 34, width: "auto" }} />
      </div>

      <div style={{ maxWidth: 540, margin: "0 auto", padding: "24px 16px 64px" }}>
        <div style={{ fontSize: 12, fontWeight: 800, letterSpacing: "0.14em", textTransform: "uppercase", color: "#04377f", textAlign: "center", marginBottom: 10 }}>
          Convite para sua equipe
        </div>

        <div style={{ background: "#fff", borderRadius: 20, padding: 14, boxShadow: "0 18px 50px -18px rgba(8,33,72,0.45)", border: "1px solid #e6eaf1" }}>
          <InvitePreview template={ev.template} data={artData} />
        </div>

        <h1 style={{ margin: "22px 0 10px", fontSize: 24, fontWeight: 800, letterSpacing: "-0.02em", textAlign: "center" }}>{ev.titulo}</h1>

        <div style={{ display: "flex", justifyContent: "center", marginBottom: 4 }}>
          <span style={{ display: "inline-flex", alignItems: "center", gap: 7, padding: "7px 14px", background: "#04377f", color: "#fff", borderRadius: 999, fontSize: 13.5, fontWeight: 700 }}>
            <Building2 size={15} /> {co.nome}
          </span>
        </div>

        {detalhes.length > 0 && (
          <div style={{ background: "#fff", border: "1px solid #e6eaf1", borderRadius: 16, padding: "6px 18px", marginTop: 16, boxShadow: "0 8px 24px -16px rgba(8,33,72,0.4)" }}>
            {detalhes.map((d, i) => {
              const Icon = d.icon
              return (
                <div key={d.label} style={{ display: "flex", alignItems: "center", gap: 13, padding: "13px 0", borderBottom: i < detalhes.length - 1 ? "1px solid #f0f2f6" : "none" }}>
                  <div style={{ width: 40, height: 40, flex: "none", borderRadius: 11, background: "#eef4fc", display: "flex", alignItems: "center", justifyContent: "center" }}>
                    <Icon size={19} color="#04377f" />
                  </div>
                  <div style={{ minWidth: 0 }}>
                    <div style={{ fontSize: 11, fontWeight: 700, color: "#9298a6", textTransform: "uppercase", letterSpacing: "0.05em" }}>{d.label}</div>
                    <div style={{ fontSize: 14.5, fontWeight: 600, color: "#1f2733" }}>{d.value}</div>
                  </div>
                </div>
              )
            })}
          </div>
        )}

        <div style={{ background: "#fff", border: "1px solid #e6eaf1", borderRadius: 18, padding: 24, marginTop: 20, boxShadow: "0 10px 30px -18px rgba(8,33,72,0.45)" }}>
          <TeamClient slug={slug} companyId={co.id} companyNome={co.nome} meta={co.convidadosPrevistos} jaCount={jaCount} />
        </div>

        <p style={{ textAlign: "center", fontSize: 11.5, color: "#9298a6", marginTop: 22 }}>
          Convite gerado por <strong style={{ color: "#6a7585" }}>Parceiro 365</strong> · Bateria 365
        </p>
      </div>
    </div>
  )
}
